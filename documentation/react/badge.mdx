...
See below Material Tailwind’s Badge examples that you can use to develop faster your website or application. These examples are coded with React and styled with Tailwind CSS. 

They come in different styles and colors, but the badge component can be customized easily so you can adapt it to your needs.
...
<DocsTitle href="default-badge">
## Default Badge
</DocsTitle>

This simple badge example is common in user interfaces to indicate the number of items or notifications associated with the button's function, like unread messages or notifications.
...
The <Code>Badge</Code> component comes with 20 different colors that you can change using the <Code>color</Code> prop.
...
In this example are 4 Badge components, each with a different color prop (<Code>"red"</Code>, <Code>"green"</Code>, <Code>"amber"</Code>, and <Code>"purple"</Code>). These props set the background color of the badge.
---
...
The <Code>placement</Code> prop values like <Code>"top-start"</Code> or <Code>"bottom-end"</Code> determine where the badge appears relative to the button—either at the top or bottom, and aligned to the start (left) or end (right) of the button.
...
You can change the overlap of the <Code>Badge</Code> component using the <Code>overlap</Code> prop. This can help place the <Code>Badge</Code> component in its right place when using it with circular elements.
...
In this example, two Badge components are used with <Code>IconButtons</Code> inside, and the other two contain an <Code>Avatar</Code> element.
...
You can add a border to the Badge component using the <Code>withBorder</Code> prop. This example shows how a badge can be enhanced visually with a border.
...
You can use the <Code>Badge</Code> component as a dot when you don't provide a <Code>content</Code> prop.
...
The dot badge is often used in UI design to indicate an active or new status, or to draw attention to the button for other reasons, without displaying a specific number or text.
...
The following props are available for the badge component. These are the custom props that we've added for the badge component and you can use all the other native props as well.
...
Our badge component is typescript-friendly. Use this to make sure that the correct prop types are passed to the component.
...
Set of predefined colors available for our Badge component. Choose from our extensive 20+ list of colors like: orange, brown, amber, yellow, blue, etc. Customize the component to fit your web project’s needs.
